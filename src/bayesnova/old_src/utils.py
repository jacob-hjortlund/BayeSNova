...
def format_property_names(
    ...
    for name in property_names:
        ...
        tmp_name = tmp_name.replace("mass", mass_name)
        tmp_name = tmp_name.replace("ssfr", ssfr_name)
...
def weighted_mean_and_error(
    values: np.ndarray, errors: np.ndarray, error_floor: float = 1e-10
) -> tuple:
    weights = 1.0 / (errors**2 + error_floor)
    mean = np.sum(values * weights) / np.sum(weights)
    err = np.sqrt(1.0 / np.sum(weights + error_floor))
...
def estimate_mmap(samples):
    ...
    output = sp_opt.minimize(
        lambda x: -kernel(x), mean_val, method="Nelder-Mead", options={"disp": False}
    ).x[0]
    t3 = time.time()
...
def create_task_name(
    ...
    cfg = yaml.safe_load(omegaconf.OmegaConf.to_yaml(cfg))
    ...
    for setting in diff_dict.get("dictionary_item_added", []):
        ...
        setting_value = functools.reduce(operator.getitem, setting_keys, cfg)
        setting_str = "_".join(setting_keys[1:]) + "-" + str(setting_value)
    ...
    for setting in diff_dict.get("dictionary_item_removed", {}):
        ...
        setting_keys = setting_keys.replace("]", "")
        setting_keys = setting_keys.split("'")[1::2]
        setting_str = "_".join(setting_keys[1:]) + "-del"
        changes.append(setting_str)
    ...
    for setting in diff_dict.get("iterable_item_removed", {}).keys():
        ...
        not_in_independent = (
            not item_removed in cfg["model_cfg"]["independent_par_names"]
        )
        ...
        if is_preset and is_pop_value and is_shared:
            setting_str = item_removed + "-" + str(fixed_value_1)
            changes.append(setting_str)
            ...
            setting_str = item_removed + "-" + str(fixed_value_no_pop)
            changes.append(setting_str)
        else:
            continue
    ...
    for setting in diff_dict.get("iterable_item_added", {}).keys():
        ...
        is_shared = (
            item_added in cfg["model_cfg"]["host_galaxy_cfg"]["shared_property_names"]
        )
        is_independent = (
            item_added
            in cfg["model_cfg"]["host_galaxy_cfg"]["independent_property_names"]
        )
        ...
        if is_shared:
            ...
            continue
    shared_host_string = "shared-" * len(shared_host_names) + "-".join(
        shared_host_names
    )
    independent_host_string = "independent-" * len(independent_host_names) + "-".join(
        independent_host_names
    )
    ...
    for setting in diff_dict.get("values_changed", {}).keys():
        ignore_setting = (
            "independent_par_name" in setting
            or "clearml_cfg" in setting
            or "data_cfg" in setting
            or "continue_from_chain" in setting
            or "n_steps" in setting
        )
        ...
        changes.append(setting_str + "-" + new_value)
...
def sigmoid(value: float, shift: float = 0.0, scale: float = 1.0, divisor: int = 1):
    exponent = (-scale * (value - shift)) / divisor
    denom = 1.0 + divisor * np.exp(exponent)
...
def extend_theta(
    theta: np.ndarray,
    n_shared_pars: int,
    n_cosmology_pars: int,
    use_physical_ratio: bool,
) -> tuple:
...
def prior_initialisation(
    priors: dict,
    preset_init_values: dict,
    shared_par_names: list,
    independent_par_names: list,
    ratio_par_name: str,
    ...
    for par in par_names:
        if par in preset_init_values.keys():
            ...
            is_gaussian = "mean" in priors[par].keys() and "std" in priors[par].keys()
            is_uniform = "lower" in priors[par].keys() and "upper" in priors[par].keys()
            if is_gaussian:
                lower = priors[par].get("lower", -np.inf)
                upper = priors[par].get("upper", np.inf)
                a = (lower - priors[par]["mean"]) / priors[par]["std"]
                b = (upper - priors[par]["mean"]) / priors[par]["std"]
                init_par = sp_stats.truncnorm.rvs(
                    a, b, loc=priors[par]["mean"], scale=priors[par]["std"]
                )
            elif is_uniform:
                bounds = priors[par]
                if len(bounds) == 2:
                ...
                raise ValueError(
                    f"{par} in prior config but bounds not defined. Check your config"
                )
    ...
    idx_independent = (
        len(init_values) - len(cosmology_par_names) - (not use_physical_ratio)
    )
    ...
    init_par_list = [shared_init_pars, independent_init_pars, cosmology_init_pars]
    ...
    try:
        ...
        idx += idx_shared
        init_pars[idx] = init_pars[idx + 1] - 1.0
    except Exception:
        pass
...
def theta_to_dict(
    ...
    shared_par_names: list,
    independent_par_names: list,
    n_independent_host_galaxy_properties: int,
    n_shared_host_galaxy_properties: int,
    ratio_par_name: str,
    cosmology_par_names: list,
    use_physical_ratio: bool,
) -> dict:
    ...
    missing_par_names = list(
        set(["gamma_Rb", "Rb", "sig_Rb", "tau_Rb", "Ebv", "tau_Ebv"])
    ...
    par_names = (
        extended_shared_par_names
        + extended_independent_par_names
        + extended_missing_par_names
        + cosmology_par_names
        + (not use_physical_ratio) * [ratio_par_name]
    )

    n_shared_pars = len(shared_par_names) + 2 * n_shared_host_galaxy_properties
    n_independent_pars = (
        2 * len(independent_par_names) + 4 * n_independent_host_galaxy_properties
    )
    n_cosmology_pars = len(cosmology_par_names)

    no_pars = (
        n_shared_pars + n_independent_pars + n_cosmology_pars + (not use_physical_ratio)
    )
    ...
    shared_pars, independent_pars = extend_theta(
        theta, n_shared_pars, n_cosmology_pars, use_physical_ratio
    )
    missing_pars = [NULL_VALUE] * len(extended_missing_par_names)
    par_list = [
        shared_pars[: 2 * n_shared_pars - 4 * n_shared_host_galaxy_properties],
        independent_pars[
            : n_independent_pars - 4 * n_independent_host_galaxy_properties
        ],
        missing_pars,
    ]
    if not use_physical_ratio:
    ...
    idx_means = np.array(
        ...
        * (n_independent_host_galaxy_properties + n_shared_host_galaxy_properties),
        dtype="bool",
    )
    idx_sigs = ~idx_means
...
def apply_sigmoid(
    arg_dict: dict, sigmoid_cfg: dict, independent_par_names: list, ratio_par_name: str
) -> dict:
    s1 = sigmoid(arg_dict[ratio_par_name], **sigmoid_cfg)
    s2 = sigmoid(
        arg_dict[ratio_par_name],
        scale=sigmoid_cfg["scale"],
        shift=1 - sigmoid_cfg["shift"],
    )
...
def vectorized_apply_sigmoid(
    chains: np.ndarray, sigmoid_cfg: dict, shared_par_names: list
):
...
def transformed_backend(
    current_backend, filename: str, name: str, sigmoid_cfg: dict, shared_par_names: list
):
    backend = em.backends.HDFBackend(filename, name=name)

    chains = vectorized_apply_sigmoid(
        current_backend.get_chain(), sigmoid_cfg, shared_par_names
    )
    ...
    for i in tqdm.tqdm(range(n)):
        ...
        state = em.State(
            coords=chains[i], log_prob=log_prob[i], random_state=random_state
        )
...
def create_gamma_quantiles(
    ...
    quantiles = np.stack((vals, sp_special.gammaincinv(vals, cdf_limit)))
